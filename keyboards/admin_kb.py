...
button_load1 = KeyboardButton('Обновить базу')
...
kb_training_description = ReplyKeyboardMarkup(resize_keyboard=True).add(add_training_description,remove_training_description).add(main_menu_button)
...
processing_buttons = ReplyKeyboardMarkup(resize_keyboard=True).add(button_reply_question,button_send_reply).add(button_change_status,button_back_admin)


button_case_admin = ReplyKeyboardMarkup(resize_keyboard=True).add(button_load2,button_load4)\
    .add(button_load3,button_load5).add(button_load1)
