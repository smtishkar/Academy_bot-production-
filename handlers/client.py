...
async def client_start(message: types.Message):
    ...
    if message.from_user.id not in test_dict:
        ...
        await message.reply('Введите ваше ID используемый на портале SABA (learn@toyota)',
...
async def load_saba_id(message: types.Message, state: FSMContext):
    ...
    if str(message.text) not in test_dict.values():
        ...
        await state.finish()
        await message.reply(
            'Вы зарегистрированы, теперь можете еще раз ввести "/start" для верификации ID номеров и доступа к '
                                            'основным функциям')
    else:
        async with state.proxy() as data:
            data['user_id'] = message.from_user.id
...
async def cm_training_request_start(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,
                           'Здесь вы можете оставить заявку для регистрации на платный тренинг. Введите информацию в свободной форме. Обязательно укажите ФИО участника, дилерский центр,'
                           'название тренинга/ов и дату начала',
